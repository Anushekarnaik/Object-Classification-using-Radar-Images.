...
class Conv3Net(nn.Module):
    def __init__(self, k1_size=(3, 3), k2_size=(3, 3), k3_size=(3, 3)):
        super(Conv3Net, self).__init__()

        # Compute dimensions after each layer
        nrows_conv1 = get_conv_output_size(11, 0, k1_size[0])  # Input size: 11
        ncols_conv1 = get_conv_output_size(61, 0, k1_size[1])  # Input size: 61

        nrows_conv2 = get_conv_output_size(nrows_conv1 // 2, 0, k2_size[0])  # MaxPool reduces size
        ncols_conv2 = get_conv_output_size(ncols_conv1 // 2, 0, k2_size[1])

        nrows_conv3 = get_conv_output_size(nrows_conv2 // 2, 0, k3_size[0])  # Another MaxPool
        ncols_conv3 = get_conv_output_size(ncols_conv2 // 2, 0, k3_size[1])

        # Ensure no negative dimensions
        if nrows_conv3 <= 0 or ncols_conv3 <= 0:
            raise ValueError(
                f"Invalid output size after convolution: nrows={nrows_conv3}, ncols={ncols_conv3}. "
                "Check input dimensions or convolution parameters."
            )

        expected_size = (nrows_conv3 // 2) * (ncols_conv3 // 2)
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(20, 40, kernel_size=k3_size),
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
        )

        # Fully connected layers
        self.fc1 = nn.Sequential(nn.Linear(40 * expected_size, 128), nn.ReLU())
        self.fc2 = nn.Sequential(nn.Linear(128, 64), nn.ReLU())
        self.fc3 = nn.Sequential(nn.Linear(64, 32), nn.ReLU())
        self.fc4 = nn.Sequential(nn.Linear(32, 3))
        self.fc_layers = [self.fc1, self.fc2, self.fc3, self.fc4]
